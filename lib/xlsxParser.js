...
function extractQTDSheet(rows) {
  ...
  for (const r of rows) {
    ...
    revCenters.push({ center, actual, ly, varD: actual - ly, varP: qtdVarP(actual, ly) });
  }

  // Sub-categories from the detail rows under each section (skip DNU/total/empties).
  const subCats = { delivery: [], pickup: [], catering: [], offsites: [] };
  for (const r of rows) {
    if (!r) continue;
    ...
    for (const [cat, prefix] of QTD_SUBCATS) {
      if (label.toLowerCase().startsWith(prefix.toLowerCase())) {
        const actual = num(r[B]), ly = num(r[H]);
        if (actual !== 0 || ly !== 0) {
          subCats[cat].push({ sub: label.slice(prefix.length).trim(), actual, ly, varD: actual - ly, varP: qtdVarP(actual, ly) });
        }
        break;
      }
    }
