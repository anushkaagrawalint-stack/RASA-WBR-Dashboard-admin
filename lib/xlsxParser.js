...
function parseWBR(wb) {
  ...
  if (revRows.length > 1) {
    ...
    for (let i = 0; i < revRows.length; i++) {
      ...
      if (!in3p30) {
        is30.push({ loc: safeStr(r[29]), reviews: num(r[30]), rating: num(r[31]), s5: num(r[32]), s4: num(r[33]), s3: num(r[34]), s2: num(r[35]), s1: num(r[36]), yelp: r[37], yelpN: num(r[38]), google: r[39], gNum: num(r[40]) });
      } else {
        tp30.push({ loc: safeStr(r[29]), reviews: num(r[30]), rating: num(r[31]), s5: num(r[32]), s4: num(r[33]), s3: num(r[34]), s2: num(r[35]), s1: num(r[36]), ue: r[37], dd: r[38], gh: r[39], errRate: num(r[40]) });
      }
    }
    W.reviews = {
...
// ── Public: parse a week from in-memory buffers (blob upload path) ───────────
// buffers: { wbr?: Buffer, loyalty?: Buffer, catering?: Buffer }
// label: the week label string (e.g. "Week of June 15")
export function parseWeekFolderFromBuffers(buffers, label) {
  let weekly={}, ptd={}, ytd={}, revCenter={}, revCenterByLoc={}, subCats={},
      subCatsByLoc={}, reviews={}, ue={}, dd={}, bikky={}, catering={};
  let loyalty={}, loyaltyMarketing=null;
  let catSales={};
  ...
  if (buffers.wbr) {
    const wb = XLSX.read(buffers.wbr, { type: 'buffer', cellDates: true });
    const W = parseWBR(wb);
    weekly = W.weekly || {};
    ptd = W.ptd || {};
    ytd = W.ytd || {};
  ...
  if (buffers.loyalty) {
    const wb = XLSX.read(buffers.loyalty, { type: 'buffer', cellDates: true });
    const L = parseLoyalty(wb);
    loyalty = L.loyalty || {};
    if (L.loyaltyMarketing) loyaltyMarketing = L.loyaltyMarketing;
  }
  ...
  if (buffers.catering) {
    const wb = XLSX.read(buffers.catering, { type: 'buffer', cellDates: true });
    catSales = parseCateringWB(wb, label);
  }
  ...
  return {
    ...
    catSales,
    qtd, qtdAvailable,
    // Signal which file types were provided — used by the UI to show/hide tabs.
    _sources: {
      wbr:      !!buffers.wbr,
      loyalty:  !!buffers.loyalty,
      catering: !!buffers.catering,
    },
  };
}
